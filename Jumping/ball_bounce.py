...
clock = pg.time.Clock()
jump = pg.mixer.Sound(path.join(CWD,"Jump_00.wav"))
...
while loop:
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT: loop = False
            ...
            if event.key == pg.K_w and (player.rect.bottom==HEIGHT): velocity.y = -20; jump.play()
            elif event.key == pg.K_w and double: velocity.y = -20; double = False; jump.play()
        elif event.type == pg.VIDEORESIZE:
            if event.w < 300: WIDTH = 300
            else: WIDTH = event.w
            if event.h < 300: HEIGHT = 300
            else: HEIGHT = event.h
            screen = pg.display.set_mode((WIDTH,HEIGHT), pg.RESIZABLE)
    ...
    if pg.sprite.collide_mask(player,box):
        if (abs(player.rect.bottom - box.rect.top) < abs(20) and player.rect.bottom!=HEIGHT) or (velocity.y > 0 and player.rect.bottom<box.rect.centery):  
            player.rect.bottom = box.rect.top; velocity.y = -20; jump.play()
            if vy<0: vy*=-1
